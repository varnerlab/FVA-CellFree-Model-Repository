...
params = vcat(TX_Vmax, plasmid_saturation_coefficient, TL_Vmax, mRNA_saturation_coefficient, mRNA_degradation_rate)

#param_bounds = [params/10 params*10]
param_bounds = [1e-9*ones(5,1) 1e6*ones(5,1)]

TXTL_dictionary["TX_Vmax"] = params[1]
TXTL_dictionary["plasmid_saturation_coefficient"] = params[2]
TXTL_dictionary["TL_Vmax"] = params[3]
TXTL_dictionary["mRNA_saturation_coefficient"] = params[4]
TXTL_dictionary["mRNA_degradation_rate"] = params[5]

alpha = 25
variance_min = 1
variance_max = 25
failed_tps_threshold = 0 #300*.05

for i in 1:1e10
	variance = exp(log(variance_min)+rand()*(log(variance_max)-log(variance_min)))
	var_round = round(variance,3)
	perturb = exp(variance*randn(length(params)))
	
	params_new = params.*perturb
	params_new = max(param_bounds[:,1],params_new)
	params_new = min(param_bounds[:,2],params_new)
	
	TXTL_dictionary_new = deepcopy(TXTL_dictionary)
	TXTL_dictionary_new["TX_Vmax"] = params_new[1]
	TXTL_dictionary_new["plasmid_saturation_coefficient"] = params_new[2]
	TXTL_dictionary_new["TL_Vmax"] = params_new[3]
	TXTL_dictionary_new["mRNA_saturation_coefficient"] = params_new[4]
	TXTL_dictionary_new["mRNA_degradation_rate"] = params_new[5]
	
	avg_perturbation = exp(mean(abs(log(params_new./params))))
	avg_perturbation_round = round(avg_perturbation,3)
	
	time_state_array_new,error_new,num_failed_tps = RunFBA(data_dictionary,TXTL_dictionary_new)
	cost_new = sum(error_new)
	
	if num_failed_tps <= failed_tps_threshold
		num_dir += 1
		mkdir("TXTL/$num_dir")
	...
	println("var = $var_round, avg_perturb = $avg_perturbation_round, num_failed_tps = $num_failed_tps")
end

#params_best = vcat(TX_Vmax, plasmid_saturation_coefficient, TL_Vmax, plasmid_saturation_coefficient, mRNA_degradation_rate)
#params = copy(params_best)

#param_bounds = [params_best/10 params_best*10]
...
#time_state_array_best,error_best,num_failed_tps = RunFBA(data_dictionary,TXTL_dictionary)
...
#			writedlm("TXTL/objective_coefficient_array",objective_coefficient_array_best)
#			writedlm("TXTL/cost_best",cost_new)
#			println("cost_new = $cost_new_round, cost = $cost_round, best = $cost_best_round, acc_prob = $acc_prob_round, var = $var_round, NEW BEST")
#		else
#			println("cost_new = $cost_new_round, cost = $cost_round, best = $cost_best_round, acc_prob = $acc_prob_round, var = $var_round")
#		end
...
#            mkdir("TXTL/$i")
#            writedlm("TXTL/$i/rate_constant.dat",rate)
#            writedlm("TXTL/$i/saturation_constant.dat",sat)
#            writedlm("TXTL/$i/control_constant.dat",cont)
#            writedlm("TXTL/$i/Tsim",Tsim)
#            writedlm("TXTL/$i/X",X)
#            writedlm("TXTL/$i/cost",cost_new)
#			# Overwrite variables
#			params = copy(params_new)
#			cost = copy(cost_new)
#			writedlm("TXTL/cost",cost_new)
#		end
#	end
