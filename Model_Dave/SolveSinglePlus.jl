...
species_array = readdlm("Results/PlusSingle/species_array")
writedlm("Results/PlusSingle/species_array",species_array[2:end,:])

species_constraint_label = species_array[1,1]
println(species_constraint_label)
mkdir("Results/PlusSingle/$species_constraint_label")

species_constraint_index = species_array[1,2:end]
species_constraint_index = find(species_constraint_index.!="")
writedlm("Results/PlusSingle/$species_constraint_label/species_constraint_index", species_constraint_index)
...
#if !(species_list_non_dataset[i] in species_to_ignore)
...
##	if !(species_list_non_dataset[j] in species_to_ignore)
...
##end
##end
#writedlm("Results/PlusSingle/species_constraint_labels",Labels)
#writedlm("Results/PlusSingle/species_array",[Labels species_constraint_array])
...
#	Accuracy[constraint_index] = accuracy
	writedlm("Results/PlusSingle/$species_constraint_label/accuracy",accuracy)
...
#	Precision[constraint_index] = precision
	writedlm("Results/PlusSingle/$species_constraint_label/precision",precision)

	# save all states
	
#	EXIT_FLAG[:,constraint_index] = Exit_flag
	writedlm("Results/PlusSingle/$species_constraint_label/Exit_flag",Exit_flag)
	writedlm("Results/PlusSingle/$species_constraint_label/Flux_array",Flux_array)
	writedlm("Results/PlusSingle/$species_constraint_label/Dual_array",Dual_array)
	writedlm("Results/PlusSingle/$species_constraint_label/Uptake_array",Uptake_array)
