...
species_list_dataset = data_dictionary["list_of_metabolite_symbols"][dataset_index]
...
KO_species_additions = [44 "M_dhap_c"; 105 "M_pep_c"; 11 "M_6pgc_c"; 138 "M_xu5p_D_c";  36 "M_cit_c"; 54 "M_fum_c"]
...
for constraint_index in 1:37
  ...
  for j in 1:length(single_fluxes)
	    ...
	    fva_max = readdlm("FVA_KO/$constraint_subset/$idx/FVA_min")
	    fva_min = readdlm("FVA_KO/$constraint_subset/$idx/FVA_max")
	...
	for j in 2:2:length(net_fluxes)
		...
		if in(idx1,rxn_list_subset)
			fva_max = readdlm("FVA_KO/$constraint_subset/$idx1/FVA_min") #min and max are switched upon saving
			fva_min = readdlm("FVA_KO/$constraint_subset/$idx2/FVA_min")
  ...
  FVA_norm = norm(FVA_MAX-FVA_MIN)
	...
	# calculate accuracyLabels
	time_state_array = readdlm("FVA_KO/$constraint_subset/time_state_array.txt")
  error_vector = CalcError(Upper,Lower,experimental_time,time_state_array,data_dictionary)
	error = sum(error_vector)

	writedlm("FVA_KO/$constraint_subset/Error_vector_centralcarbon",error_vector)
	writedlm("FVA_KO/$constraint_subset/Error_centralcarbon",error)
	writedlm("FVA_KO/$constraint_subset/FVA_Max_centralcarbon",FVA_MAX)
	writedlm("FVA_KO/$constraint_subset/FVA_Min_centralcarbon",FVA_MIN)
	writedlm("FVA_KO/$constraint_subset/FVA_norm_centralcarbon",FVA_norm)
	writedlm("FVA_KO/$constraint_subset/Labels_centralcarbon",vcat(single_flux_labels,net_flux_labels))
end
