...
time_elapsed = toc()
#=
total_accuracy = 0*ones(37)
total_precision = 0*ones(37)
total_time_state_array = zeros(146,301,37)
index = 1
for constraint_index in constraint_index_array[1:end] ;println(Labels[constraint_index])
	constraint_subset = Labels[constraint_index] #"base_case"
...
# repackage FBA data
for constraint_index in 1:15
  constraint_subset = Labels[constraint_index]
  FVA_MAX = zeros(300,194)
  FVA_MIN = zeros(300,194)
  for j in 1:194
    fva_max = readdlm("FVA_combo/$constraint_subset/$j/FVA_max")
    fva_min = readdlm("FVA_combo/$constraint_subset/$j/FVA_min")
    FVA_MAX[:,j] = fva_max[:,j]
    FVA_MIN[:,j] = fva_min[:,j]
  end
  writedlm("FVA_combo/$constraint_subset/FVA_MAX_COMBINED",FVA_MAX)
  writedlm("FVA_combo/$constraint_subset/FVA_MAX_COMBINED",FVA_MIN)
  precision = 1/norm(FVA_MAX-FVA_MIN)
  writedlm("FVA_combo/$constraint_subset/precision",precision)
end

# repackage FBA data for KO set
for constraint_index in 1:37
  constraint_subset = Labels[constraint_index]
  FVA_MAX = zeros(300,194)
  FVA_MIN = zeros(300,194)
  for j in 1:194
...
writedlm("FVA/combo_FVA_data",big_data_table_KO)
=#
total_accuracy = 0*ones(37)
total_precision = 0*ones(37)
total_time_state_array = zeros(146,301,37)
...
for constraint_index in constraint_index_array[1:end] ;println(Labels[constraint_index])
  ...
  println(constraint_index)
  total_time_state_array[:,:,constraint_index] = readdlm("FVA/$constraint_subset/time_state_array.txt")
  if !isdir("states_KO/$constraint_subset")
		mkdir("states_KO/$constraint_subset")
	end
  writedlm("states_KO/$constraint_subset/time_state_array.txt",total_time_state_array)
end

# for KO
for constraint_index in 1:37
  constraint_subset = Labels[constraint_index]
  if !isdir("/media/david/KINGSTON/updated_FVA/FVA_KO/$constraint_subset/")
		mkdir("/media/david/KINGSTON/updated_FVA/FVA_KO/$constraint_subset/")
	end
  for j in 1:194
    if !isdir("/media/david/KINGSTON/updated_FVA/FVA_KO/$constraint_subset/$j/")
  		mkdir("/media/david/KINGSTON/updated_FVA/FVA_KO/$constraint_subset/$j/")
  	end
    fva_max = readdlm("FVA_KO/$constraint_subset/$j/FVA_max")
    fva_min = readdlm("FVA_KO/$constraint_subset/$j/FVA_min")

    writedlm("/media/david/KINGSTON/updated_FVA/FVA_KO/$constraint_subset/$j/FVA_max",fva_max)
    writedlm("/media/david/KINGSTON/updated_FVA/FVA_KO/$constraint_subset/$j/FVA_min",fva_min)
  end

end
# for singles
for constraint_index in 1:25
  constraint_subset = Labels[constraint_index]
  if !isdir("/media/david/KINGSTON/updated_FVA/FVA_singles/$constraint_subset/")
		mkdir("/media/david/KINGSTON/updated_FVA/FVA_singles/$constraint_subset/")
	end
  for j in 1:194
    if !isdir("/media/david/KINGSTON/updated_FVA/FVA_singles/$constraint_subset/$j/")
  		mkdir("/media/david/KINGSTON/updated_FVA/FVA_singles/$constraint_subset/$j/")
  	end
    fva_max = readdlm("FVA_singles/$constraint_subset/$j/FVA_max")
    fva_min = readdlm("FVA_singles/$constraint_subset/$j/FVA_min")

    writedlm("/media/david/KINGSTON/updated_FVA/FVA_singles/$constraint_subset/$j/FVA_max",fva_max)
    writedlm("/media/david/KINGSTON/updated_FVA/FVA_singles/$constraint_subset/$j/FVA_min",fva_min)
  end
...
for constraint_index in 1:15
  constraint_subset = Labels[constraint_index]
  if !isdir("/media/david/KINGSTON/updated_FVA/FVA_combo/$constraint_subset/")
		mkdir("/media/david/KINGSTON/updated_FVA/FVA_combo/$constraint_subset/")
	end
  for j in 1:194
    if !isdir("/media/david/KINGSTON/updated_FVA/FVA_combo/$constraint_subset/$j/")
  		mkdir("/media/david/KINGSTON/updated_FVA/FVA_combo/$constraint_subset/$j/")
  	end
    fva_max = readdlm("FVA_combo/$constraint_subset/$j/FVA_max")
    fva_min = readdlm("FVA_combo/$constraint_subset/$j/FVA_min")

    writedlm("/media/david/KINGSTON/updated_FVA/FVA_combo/$constraint_subset/$j/FVA_max",fva_max)
    writedlm("/media/david/KINGSTON/updated_FVA/FVA_combo/$constraint_subset/$j/FVA_min",fva_min)
  end
