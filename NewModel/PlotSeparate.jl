function PlotSeparate(data_dictionary, Upper, Lower, plot_color)
...
["PROTEIN_CAT" 98 10 30 L"CAT ($\mu$M)"],
...
t_array_exp = collect(0:3.0/(size(time_state_array,2)-1):3.0)
...
for i in 1:size(carbon_species,1)
	for j in 1:size(carbon_species,2)
		...
		if !in(key,["M_axp_c";"M_gxp_c";"M_cxp_c";"M_uxp_c";"M_xxp_c"])
...
#		# Plot simulations
		if length(species_index) == 1
			sim = vec(conversion_factor*time_state_array[species_index,:])
		else
			sim = vec(conversion_factor*sum(time_state_array[species_index,:],1))
		end
		axarr_Carbon[i,j][:plot](t_array_exp,sim,plot_color,lw=1.5)
	end
end
tight_layout()
savefig("Carbon.pdf")
...
for i in 1:size(amino_species,1)
	for j in 1:size(amino_species,2)
		...
		# Plot synthetic data
		if key == "M_arg_L_c" || key == "M_glu_L_c"
...
#		# Plot simulations
		...
		axarr_Amino[i,j][:plot](t_array_exp,sim,plot_color,lw=1.5)
	end
end
tight_layout()
savefig("Amino.pdf")
...
#		# Plot simulations
		...
		axarr_Energy[i,j][:plot](t_array_exp,sim,plot_color,lw=1.5)
	end
end
tight_layout()
savefig("Energy.pdf")
