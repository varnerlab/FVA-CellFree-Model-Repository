...
#  println(mean(state_array)*(-species_drift)/tstep)
...
#species_upper_bound[144] = (.0052558/5.2-state_array[144]) / tstep
species_upper_bound[144] = (.00620859/5.2-state_array[144]) / tstep
...
#  species_upper_bound[137] = 100
  ...
  #0.0	0.0	;	# 74 M_h_c
  #0.0	0.0	;	# 75 M_h_e
  for i in 1:length(species_lower_bound)
  	if species_lower_bound[i] > species_upper_bound[i]
  		tmp = [species_lower_bound[i];species_upper_bound[i]]
  		species_lower_bound[i] = tmp[2]
  		species_upper_bound[i] = tmp[1]
  	end
  end
